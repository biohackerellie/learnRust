...
fn main() {
    ...
    let child_a = thread::spawn(move || {
        rx.iter().for_each(|msg| println!("Child A: {}", msg));
    });
    pause_ms(100);
    let child_b = thread::spawn(move || {
        let msg = rx2.recv().unwrap();
        rx2.iter().for_each(|msg| println!("Child B: {}", msg));
    });
    for i in 0..10 {
        let msg = "hello";
        println!("Main thread: Sending {} to child_a", i);
        tx.send(msg).unwrap();
        pause_ms(150);
        println!("Main thread: Sending {} to child_b", i);
        tx.send(msg).unwrap();
    }
